...
class QgisRepo(object):
    ...
    def __init__(self, args):
        ...
        self.desktop_role = getattr(args, 'role', None)
        self.auth_suffix = "-auth" \
            if (self.desktop_role is not None
    ...
    def remove_plugin_by_name(self, name):
        ...
        for p in existing_plugins:
            ...
            # Remove zip (in pre-auth0 version, the zip was kept if command
            # != 'remove')
...
class QgisPlugin(object):
    ...
    def __init__(self, args, repo):
        ...
        self.desktop_role = getattr(args, 'role', '')
    ...
    def _validate_metadata(self):
        ...
        # Add the role
        if self.desktop_role is not None:
            checked_metadata.append(('desktop_role', self.desktop_role))
    ...
    def pyqgis_plugin_element(self):
        ...
        self.add_el(el, 'desktop_role', md)
...
def arg_parser():
    ...
    roleopt = dict(action='store',
                   help='Specify the role download archive needs a particular role based authorization (implies authentication)')
