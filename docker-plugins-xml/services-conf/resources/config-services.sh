...
# Letsencrypt certs (dev and beta only!) 
sed -i "s/domain-tld-dev/${DOMAIN_TLD_DEV}/g" $NGINX_CONF/incl.d/ssl_cert_letsencrypt_dev
sed -i "s/domain-tld-beta/${DOMAIN_TLD_BETA}/g" $NGINX_CONF/incl.d/ssl_cert_letsencrypt_beta
